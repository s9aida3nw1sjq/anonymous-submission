...
from utils.train import postprocess, checkpoint_after_training_simple
from miss_methods import mean_imputation
...
def P_dis_prob(X_imputed, B_m, sigma, MLEScore):
    ...
    # coef = np.linalg.det(np.eye(d) - B_m) # Coef, here, is just a constant
...
def Adaptive_Sampling(X, Sampling_model, B_m, M_i, sigma, MLEScore, num_sampling):
    ...
    # minial_num, maximal_num = num_sampling, num_sampling
    one_run_num = 10000
...
def sigma_estimate(X_last, B_m, init=True, weight_index=None, num=None):
    if init:
        ...
        sigma = np.sqrt(
            np.sum(
                np.multiply(np.square(X_last - X_last @ B_m), weight_index), axis=0) / num
        )
    return sigma

def miss_dag_nongaussian(X, B_true, dag_init_method, dag_method, em_iter=10, MLEScore='Sup-G', num_sampling=20):
    """
    ...
    B_processed = postprocess(B_m, 0.3)
    # Checkpoint
    _logger.info("The result of MeanImputation is as follows.")
    checkpoint_after_training_simple(B_true, B_processed, _logger.info)

    # X_init_imputed = MissForest().fit_transform(X)
    # B_m = dag_init_method.fit(X_init_imputed)
    # # Post-process estimated solution
    # B_processed = postprocess(B_m, 0.3)
    # # Checkpoint
    # _logger.info("The result of Missforest is as follows.")
    # checkpoint_after_training_simple(B_true, B_processed, _logger.info)
    ...
    for m in range(1, em_iter + 1):
        ...
        ####### E-step #######
        X_sampling, num_index, weight_index = E_step(X, B_m, sigma, MLEScore, num_sampling)
        ####### E-step #######
