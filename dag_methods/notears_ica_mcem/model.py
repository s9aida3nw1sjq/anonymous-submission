...
class Model:
    ...
    def _get_mle_loss(self):
        ...
        sigma = tf.math.sqrt(
                    tf.math.reduce_sum(
                        tf.multiply(tf.square(self.X - self.X @ self.W_prime), self.weight_index), axis=0) / self.num
                    )
        if self.MLEScore == 'Sup-G':

            nm_term = 2 / self.num * tf.math.reduce_sum(
                                tf.multiply(tf.math.log(tf.math.cosh((self.X - self.X @ self.W_prime) / sigma)), self.weight_index))
        ...
        return nm_term + tf.reduce_sum(sigma) - tf.linalg.slogdet(tf.eye(self.d) - self.W_prime)[1]