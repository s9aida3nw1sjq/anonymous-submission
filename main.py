...
from dag_methods import Golem, Ghoshal, Notears, Notears_ICA_MCEM, \
...
def main():
    ...
    if args.dag_method_type == 'golem':
        dag_method = Golem(args.lambda_1_ev, args.lambda_1_nv, args.lambda_2,
                           args.equal_variances, args.golem_iter,
                           args.learning_rate, args.seed)
    elif args.dag_method_type == 'notears':
    ...
    if args.miss_method_type == 'miss_dag_gaussian':
        B_est, cov_est, histories = miss_dag_gaussian(dataset.X, dataset.mask, dag_method,
        ...
        B_est, cov_est, histories = miss_dag_nongaussian(dataset.X, dataset.B, dag_init_method,
                                                         dag_method, args.em_iter, args.MLEScore, args.num_sampling)
        ...
        B_est, cov_est, histories = miss_dag_nonlinear(dataset.X, dataset.B, dag_init_method,
    ...
    # Checkpoint
    checkpoint_after_training(output_dir, dataset.X_true, dataset.mask, dataset.Omega, dataset.B,
                              B_est, B_processed, cov_est, histories, _logger.info)
